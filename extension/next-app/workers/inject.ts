...
window.addEventListener("message", (event) => {
  ...
  if (event?.data?.type === "WALLET_CONTENT") {
    event.data.data.data.resId = event.data.resId;
    event.data.data.data.type = "WALLET_CONTENT_MSG";
    ...
    if ((event?.data?.data?.method ?? "x") in allowedMethods) {
      ...
      chrome?.runtime?.sendMessage(event.data.data.data, (res) => {
        ...
        const id = Number(
          event.data.resId.replace(/[A-Za-z]/g, "").slice(0, 10)
        );
        const data = {
  ...
  } else if (event?.data?.type === "WALLET_PING") {
    event.data.data.data.resId = event.data.resId;
    event.data.data.data.type = "WALLET_CONTENT_MSG";
...
chrome.runtime.onMessage.addListener((message: any, sender, sendResponse) => {
  ...
  if (message.type === "WALLET_EXT_LISTNER") {
    const data = { type: "WALLET_PAGE_LISTENER", data: message.data };
    window.postMessage(data, "*");
  }
  return true;
});
